...
internal class PrestamoRepository : IPrestamoRepository
{
    ...
    public async Task<Prestamo?> GetByIdAsync(long id, CancellationToken ct)
    ...
    public async Task<IReadOnlyList<Prestamo>> GetByUserAsync(long userId, CancellationToken ct)
    ...
    public async Task<IReadOnlyList<Prestamo>> GetAllAsync(CancellationToken ct)
     => await _db.Set<Prestamo>().Include(x => x.UsuarioCreacion).Include(x => x.UsuarioModificacion).OrderByDescending(l => l.FechaCreacion).ToListAsync(ct);

    public async Task<IReadOnlyList<Prestamo>> GetPendingAsync(CancellationToken ct)
        => await _db.Set<Prestamo>().Include(x => x.UsuarioCreacion).Include(x=> x.UsuarioModificacion).Where(l => l.Estado == EstadoPrestamo.pendiente).OrderBy(l => l.FechaCreacion).ToListAsync(ct);
